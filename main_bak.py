...
from src.migration.data_migrator import migrate_data, move_user_data
from src.migration.direct_migration import direct_migrate, resume_direct_migration
from src.migration.integrity_checker import check_integrity
...
def main():
    ...
    #logger.info(f"Загруженный конфиг файл: {config}")
    ...
    try:
        ...
        # Получение пути к исходной папке с данными текущего компьютера
        ...
        try:
            for user in users:
                try:
                    ...
                    # Определение путей
                    buffer_dir = os.path.join(config["TARGET_BUFFER_DIRECTORY"], linux_user)

                    # Копирование скелета в папку пользователя
                    copy_skel(final_target_dir)
                    # Копирование данных
                    migrate_data(user_dir, buffer_dir, exclude_dirs=config["EXCLUDE_DIRS"], exclude_files=config["EXCLUDE_FILES"], username=linux_user, report_data=report_data)

                    # Проверка целостности данных
                    integrity_ok = check_integrity(user_dir, buffer_dir, discrepancies_file=mismatch_file, report_data=report_data)

                    # Перемещение данных в целевую директорию
                    move_user_data(buffer_dir, final_target_dir)
                    logger.info(f'Данные успешно перенесены в {final_target_dir}')
                    
                    # Обработка ярлыков после перемещения данных
                    ...
                    if integrity_ok:
                        ...
                        logger.warning(f"Миграция пользователя {linux_user} завершена с несоответствиями. См. отчёт для подробностей.")
