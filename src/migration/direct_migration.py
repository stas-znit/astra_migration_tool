...
def direct_copy_file(source_file, target_file, source_dir, target_dir, username=None, report_data=None, lock=None, no_mapping=True):
    ...
    try:
            ...
            try:
                ...
                if "No space left" in str(e):
                    handle_migration_error(
                        ...
                        details=f"Недостаточно места для создания директории: {target_dir_path}",
                        exception=e,
                        context={"user": username, "target_dir": target_dir_path, "source_file": source_file}
                    )
                else:
                    handle_migration_error(
                        MigrationErrorCodes.TARGET_001,
                        details=f"Не удалось создать целевую директорию: {target_dir_path}",
                        exception=e,
                        context={"user": username, "target_dir": target_dir_path}
                    )
        ...
        if not os.path.exists(target_file_short) or os.path.getmtime(source_file) > os.path.getmtime(target_file_short):
            ...
            try:
                ...
                handle_migration_error(
                    ...
                    details=f"Нет прав для записи файла: {target_file_short}",
                    exception=e,
                    context={"user": username, "source_file": source_file, "target_file": target_file_short}
                )
                ...
                if "No space left" in str(e):
                    handle_migration_error(
                        MigrationErrorCodes.TARGET_002,
                        details=f"Недостаточно места для копирования файла: {os.path.basename(source_file)}",
                        exception=e,
                        context={"user": username, "source_file": source_file, "file_size": os.path.getsize(source_file)}
                    )
                elif "File name too long" in str(e):
                    handle_migration_error(
                        MigrationErrorCodes.COPY_003,
                        details=f"Имя файла слишком длинное: {os.path.basename(source_file)}",
                        exception=e,
                        context={"user": username, "filename": target_basename}
                    )
                else:
                    handle_migration_error(
                        MigrationErrorCodes.COPY_001,
                        details=f"Ошибка копирования файла: {os.path.basename(source_file)}",
                        exception=e,
                        context={"user": username, "source_file": source_file, "target_file": target_file_short}
                    )
                ...
                handle_migration_error(
                    ...
                    details=f"Исходный файл не найден: {os.path.basename(source_file)}",
                    exception=e,
                    context={"user": username, "source_file": source_file}
                )
                return False, f"Исходный файл не найден: {e}"
            ...
            if not integrity_ok:
                ...
                handle_migration_error(
                    MigrationErrorCodes.VERIFY_001,
                    details=f"Несовпадение целостности файла: {os.path.basename(source_file)}",
                    context={"user": username, "source_file": source_file, "target_file": target_file_short}
                )
        ...
        handle_migration_error(
            MigrationErrorCodes.COPY_001,
            details=f"Неожиданная ошибка при копировании файла: {os.path.basename(source_file)}",
            exception=e,
            context={"user": username, "source_file": source_file, "target_file": target_file}
        )
...
def verify_file_integrity(source_file, target_file, source_dir, target_dir, username=None):
    ...
    try:
        if integrity_check_method == 'hash':
            ...
            if preloaded_hashes:
                ...
                expected_hash_paths = [
                    ...
                    clean_username + "\\" + rel_path.replace('/', '\\'),
                ...
                if expected_hash:
                    ...
                    try:
                        target_hash = calculate_file_hash(target_file, algorithm=config.get("HASH_ALGORITHM", "sha256"))
                        if target_hash and expected_hash:
                            return target_hash.lower() == expected_hash.lower()
                        else:
                            handle_migration_error(
                                MigrationErrorCodes.VERIFY_002,
                                details=f"Не удалось вычислить хеш для файла: {os.path.basename(target_file)}",
                                context={"user": username, "target_file": target_file, "algorithm": config.get("HASH_ALGORITHM", "sha256")}
                            )
                            return False
                    except Exception as e:
                        handle_migration_error(
                            MigrationErrorCodes.VERIFY_002,
                            details=f"Ошибка при вычислении хеша файла: {os.path.basename(target_file)}",
                            exception=e,
                            context={"user": username, "target_file": target_file}
                        )
                        return False
                    

            # Если нет предзагруженных хешей или хеш не найден, вычисляем хеши напрямую (СУЩЕСТВУЮЩИЙ КОД)
            try:
                ...
                if source_hash is None or target_hash is None:
                    handle_migration_error(
                        MigrationErrorCodes.VERIFY_002,
                        details=f"Не удалось вычислить хеш для файлов при сравнении",
                        context={"user": username, "source_file": source_file, "target_file": target_file}
                    )
                ...
                handle_migration_error(
                    MigrationErrorCodes.VERIFY_002,
                    details=f"Ошибка при вычислении хешей для сравнения",
                    exception=e,
                    context={"user": username, "source_file": source_file, "target_file": target_file}
                )
                return False
            ...
            try:
                ...
                handle_migration_error(
                    MigrationErrorCodes.VERIFY_003,
                    details=f"Ошибка при сравнении размеров файлов",
                    exception=e,
                    context={"user": username, "source_file": source_file, "target_file": target_file}
                )
                return False
            ...
            try:
                ...
                handle_migration_error(
                    MigrationErrorCodes.VERIFY_002,
                    details=f"Ошибка при сравнении метаданных файлов",
                    exception=e,
                    context={"user": username, "source_file": source_file, "target_file": target_file}
                )
                return False
            ...
            handle_migration_error(
                ...
                details=f"Неизвестный метод проверки целостности: {integrity_check_method}",
                context={"user": username, "integrity_method": integrity_check_method}
            )
        ...
        handle_migration_error(
            MigrationErrorCodes.VERIFY_002,
            details=f"Неожиданная ошибка при проверке целостности файла",
            exception=e,
            context={"user": username, "source_file": source_file, "target_file": target_file, "method": integrity_check_method}
        )
...
def rename_directories(target_dir, folder_mapping, desktop_rename, username=None):
    ...
    try:
        ...
        for folder_name, new_path in desktop_rename.items():
            ...
            if os.path.exists(old_path) and os.path.isdir(old_path):
                ...
                try:
                    ...
                    handle_migration_error(
                        MigrationErrorCodes.TARGET_003,
                        details=f"Нет прав для переименования директории: {folder_name}",
                        exception=e,
                        context={"user": username, "old_path": old_path, "new_path": new_full_path}
                    )
                    return False
                except OSError as e:
                    if "No space left" in str(e):
                        handle_migration_error(
                            MigrationErrorCodes.TARGET_002,
                            details=f"Недостаточно места для переименования директории: {folder_name}",
                            exception=e,
                            context={"user": username, "folder_name": folder_name}
                        )
                    else:
                        handle_migration_error(
                            MigrationErrorCodes.SYSTEM_002,
                            details=f"Системная ошибка при переименовании директории: {folder_name}",
                            exception=e,
                            context={"user": username, "old_path": old_path, "new_path": new_full_path}
                        )
                    return False
        ...
        for folder_name, new_name in folder_mapping.items():
            ...
            if os.path.exists(old_path) and os.path.isdir(old_path):
                ...
                try:
                    ...
                    handle_migration_error(
                        MigrationErrorCodes.SYSTEM_002,
                        details=f"Ошибка при переименовании директории: {folder_name}",
                        exception=e,
                        context={"user": username, "old_path": old_path, "new_path": new_path}
                    )
                    # Продолжаем работу, не останавливаемся на одной ошибке
                    logger.warning(f"Ошибка переименования {folder_name}, продолжаем работу")
        ...
        handle_migration_error(
            MigrationErrorCodes.SYSTEM_002,
            details=f"Критическая ошибка при переименовании директорий",
            exception=e,
            context={"user": username, "target_dir": target_dir}
        )
...
def direct_migrate(source_dir, target_dir, exclude_dirs=None, exclude_files=None, username=None, report_data=None):
    ...
    if config.get("INTEGRITY_CHECK_METHOD") == 'hash' and config.get("DATABASE_PATH"):
        ...
        try:
            ...
            handle_migration_error(
                MigrationErrorCodes.SYSTEM_001,
                details=f"Ошибка при загрузке хешей из базы данных",
                exception=e
            )
    ...
    try:
        ...
        try:
                ...
                for file in files:
                    ...
                    try:
                        ...
                        files_to_copy.append((mtime, source_file, dest_file, file_size))
                            ...
                            # *** НОВОЕ: Код ошибки для недоступного файла ***
                        handle_migration_error(
                            MigrationErrorCodes.SOURCE_003,
                            details=f"Файл недоступен во время сканирования: {file}",
                            exception=e,
                            context={"user": username, "source_file": source_file}
                        )
                        continue
                    except Exception as e:
                            # *** НОВОЕ: Общий код ошибки для сканирования ***
                        handle_migration_error(
                            MigrationErrorCodes.SOURCE_003,
                            details=f"Ошибка при получении информации о файле: {file}",
                            exception=e,
                            context={"user": username, "source_file": source_file}
                        )
            ...
            # *** НОВОЕ: Код ошибки для доступа к исходной директории ***
            handle_migration_error(
                MigrationErrorCodes.SOURCE_001,
                details=f"Нет прав для чтения исходной директории",
                exception=e,
                context={"user": username, "source_dir": source_dir}
            )
            return False
        except Exception as e:
            # *** НОВОЕ: Общий код ошибки для сканирования ***
            handle_migration_error(
                MigrationErrorCodes.SOURCE_003,
                details=f"Ошибка при сканировании исходной директории",
                exception=e,
                context={"user": username, "source_dir": source_dir}
            )
            return False
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    handle_migration_error(
                        ...
                        details=f"Ошибка при обработке результата многопоточного копирования",
                        exception=e,
        ...
        if copy_success:
            ...
            if report_data and report_data.get('discrepancies'):
                try:
                    ...
                    # *** НОВОЕ: Код ошибки для записи файла несовпадений ***
                    handle_migration_error(
                        MigrationErrorCodes.TARGET_001,
                        details=f"Не удалось записать файл несовпадений",
                        exception=e,
                        context={"user": username, "discrepancies_file": discrepancies_file}
                    )
        ...
        handle_migration_error(
            MigrationErrorCodes.SYSTEM_003,
            details=f"Критическая ошибка при выполнении прямой миграции",
            exception=e,
            context={"user": username, "source_dir": source_dir, "target_dir": target_dir}
        )
...
def save_migration_state(username):
    ...
    try:
        ...
        handle_migration_error(
            MigrationErrorCodes.TARGET_003,
            details=f"Нет прав для сохранения состояния миграции",
            exception=e,
            context={"user": username, "state_file": state_file}
        )
    except OSError as e:
        if "No space left" in str(e):
            handle_migration_error(
                MigrationErrorCodes.TARGET_002,
                details=f"Недостаточно места для сохранения состояния миграции",
                exception=e,
                context={"user": username}
            )
        else:
            handle_migration_error(
                MigrationErrorCodes.SYSTEM_002,
                details=f"Системная ошибка при сохранении состояния миграции",
                exception=e,
                context={"user": username, "state_file": state_file}
            )
    except Exception as e:
        handle_migration_error(
            MigrationErrorCodes.SYSTEM_002,
            details=f"Ошибка при сохранении состояния миграции",
            exception=e,
            context={"user": username}
        )
...
def load_migration_state(username):
    ...
    try:
        ...
        handle_migration_error(
            MigrationErrorCodes.SOURCE_001,
            details=f"Нет прав для чтения файла состояния миграции",
            exception=e,
            context={"user": username, "state_file": state_file}
        )
        return {}
        ...
        handle_migration_error(
            MigrationErrorCodes.SOURCE_003,
            details=f"Файл состояния миграции поврежден",
            exception=e,
            context={"user": username, "state_file": state_file}
        )
        return {}
        ...
        handle_migration_error(
            MigrationErrorCodes.SOURCE_003,
            details=f"Ошибка при загрузке состояния миграции",
            exception=e,
            context={"user": username}
        )
